...
def setup_prod(project: str, sites, email: str, version: str = None, image=None, pull_policy: str = "if_not_present") -> None:
    if len(sites) == 0:
    ...
    if check_repo_exists():
        ...
        with open(compose_file_name, "w") as f:
            ...
            try:
                subprocess.run(
                    ...
                    stdout=f,
                    check=True,
                    env={**os.environ, 'PULL_POLICY': pull_policy}
                )
            except Exception:
        ...
        os.environ['PULL_POLICY'] = pull_policy
        print(f"PULL_POLICY: {os.environ.get('PULL_POLICY', 'not set')}")
        try:
            subprocess.run(
                [
                    which("docker"),
        ...
        setup_prod(project, sites, email, version, image, pull_policy)
...
def setup_dev_instance(project: str):
    if check_repo_exists():
        try:
            ...
            cprint(
                "Please go through the Development Documentation: Developed by Vohala.",
                level=2,
            )
...
def install_docker():
    ...
    if platform.system() == "Darwin" or platform.system() == "Windows":
        print("Docker Installation failed for Mac or Windows.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Install Developed by Vohala")
    parser.add_argument(
    ...
    parser.add_argument(
        "--pull-policy",
        help="Set Docker image pull policy (always, never, if_not_present, build, missing). Defaults to 'if_not_present'.",
    ...
    if args.dev:
        ...
        if "example.com" in args.email:
            ...
            sys.exit(1)
        setup_prod(args.project, args.sites, args.email, args.version, args.image, args.pull_policy)
    else:
        parser.print_help()
